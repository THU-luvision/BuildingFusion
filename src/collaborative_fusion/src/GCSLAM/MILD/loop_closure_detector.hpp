...
namespace MILD
{
    ...
    public:
      ...
      LCDCandidate(float input_salient_score, int input_index,int _submapID, int _cameraID) :salient_score(input_salient_score), index(input_index), submapID(_submapID), cameraID(_cameraID){}
      float salient_score;
      int index;
      int submapID;
      int cameraID;
...
EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    ...
    int construct_database(cv::Mat desc,int frame_index,int frame_submapID, int frame_cameraID)
    {
      ...
      submapID.push_back(frame_submapID);
      cameraID.push_back(frame_cameraID);
      return features_descriptor.size();
    }
    //when we came out of an room, we reset the mild. these can be used to reject some of the false loop closure detection
    
    void reset()
    {
      submapID.clear();
      cameraID.clear();
      features_descriptor.clear();
      index.clear();
      features_buffer = std::vector<mild_entry>(entry_num_per_hash_table * hash_table_num);
      for (int i = 0; i < entry_num_per_hash_table * hash_table_num; i++)
      {
