                       ...
                       std::vector<int> &keyframesToUpdate)
{
  ...
  //      std::cout << "average: " << i << " " << movingAverageList[i] << std::endl;
    }
    int max_index = 0;
    float max_value = -1;
 ...
 //   std::cout << "max value: " << max_index << " " << max_value << std::endl;
    ...
    if(max_value > threshold)
    {
        ...
        for(int i = 0; i < keyframeCostList.size();i++)
        {
            ...
            {
 //               std::cout << "chosen max value: " << keyframeCostList[i] << " " << keyframeIDList[i] << std::endl;
