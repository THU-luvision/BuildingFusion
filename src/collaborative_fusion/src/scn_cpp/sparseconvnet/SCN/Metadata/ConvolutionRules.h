...
template <Int dimension>
void point_sort(std::vector<Point<dimension>> &a)
{
...
/*
  ...
  gpuErrchk(cudaFree(d_next_all_point));
  std::vector<Point<dimension>> debug_outputpoint;
  // Point<dimension> p;
    for (Int i = 0; i < nActiveInput; i++)  {
      for (Int j = 0; j < maxi_sizec; j++)
      {
...
*/
  ...
  EASY_BLOCK("query");
  // can also be more efficient, but it's more complex because of the struct rulebook std::vector
  Int query_index = 0;
  for (Int i = 0; i < nActiveInput; i++)  {
    for(Int k = 0; k < dimension; k++)  {
      p[k] = in_points_flat[i + k * nActiveInput];
